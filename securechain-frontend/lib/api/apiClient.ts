...
import type { DefectDojoImportRequest } from '@/types/DefectDojo'
...
export const defectdojoAPI = {
  generateFromRepo: async (owner: string, repository: string) => {
    return apiClient.post(
      API_ENDPOINTS.DEFECTDOJO.GENERIC_FINDINGS.GENERATE,
      { owner, repository },
      { retries: 0 }
    )
  },
  ...
  listDocuments: async () => {
    return apiClient.get(
      API_ENDPOINTS.DEFECTDOJO.GENERIC_FINDINGS.DOCUMENTS
    )
  },

  getDocument: async (documentId: string) => {
    return apiClient.get(
      API_ENDPOINTS.DEFECTDOJO.GENERIC_FINDINGS.DOCUMENT_DETAIL(documentId)
    )
  },
  ...
  return apiClient.post(
    API_ENDPOINTS.DEFECTDOJO.GENERIC_FINDINGS.IMPORT(documentId),
    payload ?? {},
    { retries: 0 }
  )
}
